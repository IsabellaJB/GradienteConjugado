...
def newton(funcion_objetivo, x, epsilon1, epsilon2, max_iterations, alpha):
    ...
    while not terminar:
        # Step 2: Calcular el gradiente
        gradienteX = np.array(gradiente(funcion_objetivo, xk))

        # Step 3: Calcular la Hessiana y su inversa
        ...
        # Step 5: Encontrar α^(k) minimizando la función
        ...
        # Step 6: Comprobar condiciones de terminación
        distancia = distancia_origen(gradienteX)
        if distancia <= epsilon1 or (distancia_origen(x_k1 - xk) / (distancia_origen(xk) + 0.00001)) <= epsilon2 or k >= max_iterations:
            terminar = True
        else:
            k = k + 1
            xk = x_k1
